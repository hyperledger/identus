...
This repository uses automated file hygiene checks via CI. Before submitting a PR, run the lint script to catch and fix formatting issues:
...
- **[markdownlint-cli2](https://github.com/DavidAnson/markdownlint-cli2)** — Validates and auto-fixes `.md` files (table alignment, blank lines, trailing spaces). Configuration: `.markdownlint.yml`.
- **[yamllint](https://github.com/adrienverge/yamllint)** — Validates `.yml`/`.yaml` files. Configuration: `.yamllint.yml`.
- **[editorconfig-checker](https://github.com/editorconfig-checker/editorconfig-checker)** — Validates all text files for charset (UTF-8 without BOM), line endings (LF), trailing whitespace, and indentation. Configuration: `.editorconfig`.
- **[ShellCheck](https://www.shellcheck.net/)** — Static analysis for `.sh` files.

Both VS Code (with the [EditorConfig extension](https://marketplace.visualstudio.com/items?itemName=EditorConfig.EditorConfig)) and IntelliJ IDEA natively respect `.editorconfig` for indent style, charset, and line endings.
...
| triage:stale | For issues that are seen once or have no paradigm to reproduce. The triage will then decide what action to do in order to progress. | Triage team adds a comment to describe the next steps and `Assignees` is set accordingly |
...
| triage:out-of-scope | Indicates the implementation is following the specification or an existing ADR and awaiting confirmation by triage | If triage is ok, the issue is closed |
...
| type:roadmap | This issue will appear in the project roadmap | |
...
The documentation is maintained in the [hyperledger-identus/docs](https://github.com/hyperledger-identus/docs) repository. It is automatically published to GitHub Pages on each commit to the `main` branch — no manual deployment steps are required.
...
- **Discord** - Linux Foundation Decentralized Trust server: <https://discord.gg/MhzprcfP75>
